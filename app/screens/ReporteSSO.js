...
export default function InformeInspeccionScreen() {
  ...
  const [fechaCorreccion, setFechaCorreccion] = useState(new Date());
  const [hallazgo, setHallazgo] = useState('');
  const [recomendacion, setRecomendacion] = useState('');
  const [hallazgos, setHallazgos] = useState([]);
  const [imageBase64, setImageBase64] = useState('');
  const [showDatePickerHallazgo, setShowDatePickerHallazgo] = useState(false);
  const [showDatePickerCorreccion, setShowDatePickerCorreccion] = useState(false);
      ...
      .map(
       ...
       <tr>
            ...
            <tr>
                ...
                <td style="width: 25%; text-align: center;"><img src="data:image/png;base64,${h.imageBase64}" alt="Corrección" class="evidence-image"></td>
            </tr>`
      )
      .join('');
    ...
    <div class="container">
        ...
        <table class="header-table">
            <tr>
                ...
                <td class="left-align" style="width: 25%;">FECHA DE CORRECCIÓN: ${fechaCorreccion.toLocaleDateString()}</td>
            </tr>
            <tr>
...
`;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.datePickerContainer}>
        <Button
          title={`Fecha de Corrección: ${fechaCorreccion.toLocaleDateString()}`}
          onPress={() => setShowDatePickerCorreccion(true)}
          color={styles.button.color}
        />
        {showDatePickerCorreccion && (
          <DateTimePicker
            value={fechaCorreccion}
            mode="date"
            display="default"
            onChange={(event, selectedDate) => {
              setShowDatePickerCorreccion(false);
              if (selectedDate) {
                setFechaCorreccion(selectedDate);
              }
            }}
          />
        )}
